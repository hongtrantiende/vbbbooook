...
### ADR-009: Bypass Premium Check khi xuất Ebook và ép luôn gộp file TXT trong smali bytecode

**Date:** 2026-06-08
**Status:** Accepted

**Context:** 
Người dùng muốn sử dụng chức năng xuất ebook định dạng TXT mà không bị chặn bởi thông báo yêu cầu tài khoản Premium, đồng thời các chương sách phải được gộp chung lại thành một file TXT duy nhất thay vì chia nhỏ thành zip.

**Options Considered:**
1. **Chỉnh sửa UI State và biến cấu hình** - Cố gắng thay đổi các UI bindings. Tuy nhiên, logic check và gộp file nằm sâu trong các tiến trình bất đồng bộ (Actions/Workers), dễ bị ghi đè bởi luồng dữ liệu từ Compose.
2. **Can thiệp trực tiếp vào Bytecode (smali patch)** - Sửa đổi trực tiếp trong `f91.smali` để comment out nhánh rẽ hiển thị lỗi Premium (`cond_b`), và sửa `js3.smali` (constructor của cấu hình xuất `ExportBookConfig`) để luôn ép trường `g` (`mergeIntoSingleFile`) thành `true`.

**Decision:**
Chọn **phương án 2** nhằm đảm bảo tính năng hoạt động độc lập với trạng thái UI, triệt để và hoạt động offline hoàn toàn.

**Consequences:**
- ✅ Mọi tài khoản, dù là Premium hay không, đều có thể xuất ebook thành công mà không gặp lỗi.
- ✅ Khi xuất file TXT hoặc HTML, các chương sách luôn được gộp tự động thành 1 file duy nhất.
- ➖ Người dùng bắt buộc phải gỡ cài đặt app cũ trên thiết bị trước khi cài đặt APK mới để tránh bị cache chữ ký/dữ liệu cũ làm mất tác dụng của patch.
